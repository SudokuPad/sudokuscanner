const PuzzleTools = (() => {
	...
	// classic codec
		...
		PT.unzip = zipped => {
			if(PT.isClassic(zipped)) return PT.unzipClassicSudoku(zipped);
			if(PT.isClassic2(zipped)) return PT.unzipClassicSudoku2(zipped);
		};
	// format checking
		PT.isJson = data => {
		...
		PT.isClassic2 = (data = '') => {
			if(!data.match(new RegExp(`^[${blankEncodes}]+$`))) return false;
